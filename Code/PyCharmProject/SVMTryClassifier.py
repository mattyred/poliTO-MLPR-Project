...
def modifiedDualFormulation(DTR, LTR, C, K):
    ...
    (x, f, d) = scipy.optimize.fmin_l_bfgs_b(LD_objectiveFunctionOfModifiedDualFormulation,
                                             np.zeros(DTR.shape[1]), args=(Hij,), bounds=b, iprint=0, factr=1.0)
...
class SVM():
    ...
    def predict(self, DTE):
        ...
        if (self.option == 'polynomial'):
            S = np.sum(
                np.dot((self.x * self.LTR).reshape(1, self.DTR.shape[1]),
                       (np.dot(self.DTR.T, DTE) + self.c) ** self.d + self.K), axis=0)
            LP = 1 * (S > 0)
            LP[LP == 0] = -1
            return LP
        if (self.option == 'RBF'):
