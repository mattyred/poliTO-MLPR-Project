...
\begin{table}[ht!]
		...
		\begin{tabular}{ |l|l|l|l| }
			...
			\hline
			\multirow{3}{*}{}
			 LLR \scriptsize{($\lambda=10^{-3}$)}& \textcolor{blue}{0.170} & 0.170 & 0.155\\
			...
			\hline
			\multirow{3}{*}{}
			 LLR \scriptsize{($\lambda=10^{-3}$)}& 0.299 & \textcolor{red}{0.113} & 0.263\\
...
\underline{Selected LLR Model}: Z-normalized features, $\lambda=10^{-5}$, no PCA
...
\begin{table}[ht!]
	...
	\begin{tabular}{ |l|l|l|l| }
		...
		\hline
		\multirow{1}{*}{}
		LLR &&&\\\scriptsize{(Z-Norm, $\lambda=10^{-5}$, no PCA)}& 0.132 & 0.047 & 0.127\\
		\hline
	\end{tabular}
\end{table}
...
\\
\underline{Selected QLR Model}: Z-Normalized features, $\lambda=10^{-5}$, no PCA
...
\begin{figure}[ht!]
	...
	\caption{Polynomial SVM (K=1, c=0, d=2) - minDCF for different values of C}
	\label{polysvm} 
\end{figure}
...
\begin{table}[ht!]
	\caption{Polynomial Kernel SVM - C=1}
	\centering
	\begin{tabular}{ |l|l|l|l| }
		\hline
		& $\tilde{\pi}=0.1$ & $\tilde{\pi}=0.5$ & $\tilde{\pi}=0.9$ \\ \hline
		...
		\multirow{2}{*}{}
		 ...
		 Poly SVM \scriptsize{(c=1, d=2)}& 0.187 & 0.060 & 0.164\\
		 Poly SVM \scriptsize{(c=1, d=3)}& 0.408 & 0.157 & 0.563\\
		\hline
		\multicolumn{4}{ |c| }{\bf{Raw features - PCA(m=11)}} \\
		\hline
		\multirow{2}{*}{}
		Poly SVM \scriptsize{(c=0, d=2)}& 0.553 & 0.199 & 0.487\\
		Poly SVM \scriptsize{(c=1, d=2)}& 0.210 & 0.062 & 0.158\\
		...
		Poly SVM \scriptsize{(c=1, d=2)}& 0.465 & 0.158 & 0.396\\
		Poly SVM \scriptsize{(c=1, d=3)}& 0.784 & 0.280 & 0.815\\
		\hline
	\end{tabular}
\end{table}
The other kernel that was emplyed is the RBF kernel and these are the results achieved. In this case
...
so i trained the model for different values of $\lambda$ to analyze the performances
\begin{figure}[ht!]
	\includegraphics[width=\linewidth]{./Pictures/FeaturesAnalysis/rbfsvm_znorm.png}
	\caption{RBF SVM ($K=1, \gamma=0.1$) - minDCF for different values of C}
	\label{rbfsvm_znorm} 
\end{figure}
\begin{figure}[ht!]
	\includegraphics[width=\linewidth]{./Pictures/FeaturesAnalysis/rbfsvm_g001_znorm.png}
	\caption{RBF SVM ($K=1, \gamma=0.01$) - minDCF for different values of C}
	\label{rbfsvm_g001_znorm} 
\end{figure}
\begin{table}[ht!]
	...
	\begin{tabular}{ |l|l|l|l| }
		...
		\hline
		\multirow{3}{*}{}
		 RBF SVM \scriptsize{($\gamma=1$)}& 0.291 & 0.095 & 0.281\\
		 RBF SVM \scriptsize{($\gamma=0.1$)}& 0.163 & 0.056 & 0.137\\
		 RBF SVM \scriptsize{($\gamma=0.01$)}& 0.153 & 0.049 & 0.133\\
		\hline
		\multicolumn{4}{ |c| }{\bf{Z-Normalized features - PCA(m=11)}} \\
		\hline
		\multirow{3}{*}{}
		RBF SVM \scriptsize{($\gamma=1$)}& 0.362 & 0.123 & 0.350\\
		RBF SVM \scriptsize{($\gamma=0.1$)}& 0.255 & 0.092 & 0.255\\
		RBF SVM \scriptsize{($\gamma=0.01$)}& 0.254 & 0.091 & 0.211\\
		\hline
		\multicolumn{4}{ |c| }{\bf{Z-Normalized features - PCA(m=10)}} \\
		\hline
		\multirow{3}{*}{}
		RBF SVM \scriptsize{($\gamma=1$)}& 0.386 & 0.136 & 0.379\\
		RBF SVM \scriptsize{($\gamma=0.1$)}& 0.271 & 0.106 & 0.273\\
